...
void shanchu(struct headnode* currenthead,int a)
{
	...
	if (p == currenthead->first)
	{
		currenthead->first = p->next;
		p->next = NULL;
	}
	else if(p->next==NULL&&p != currenthead->first)
	{
		p->prev->next=NULL;
		p->next=p->prev=NULL;
	}
